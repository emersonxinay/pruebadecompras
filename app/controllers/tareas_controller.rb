class TareasController < ApplicationController
  before_action :set_tarea, only: [:completar, :show, :edit, :update, :destroy]
  ...
  # GET /tareas/1.json
  def show
    @ranking = @tarea.users.order(:created_at).limit(5)
  end
  ...
  def completar
      ...
      if (@tarea.users.include?(current_user))
        @tarea.users.destroy(current_user)
      else
        @tarea.users << current_user
      end
      @tarea.save
  ...
  private
    ...
    def tarea_params
      params.require(:tarea).permit(:title, :desc, :photo)
    end
end
